...
<div class="bg-panel rounded-lg border border-steel flex flex-col hover:border-brand-700 transition">
    <div class="p-6 flex-1 flex flex-col">
        <div class="flex items-center gap-2">
            <h3 class="text-lg font-bold text-paper">{{ $package->name }}</h3>
            @if ($package->is_trial)
        ...
        @if ($package->featureList())
            <ul class="mt-4 space-y-2 text-sm text-dim flex-1">
                @foreach ($package->featureList() as $feature)
                    <li class="flex items-start gap-2">
                        <svg class="h-5 w-5 shrink-0 text-brand-400" viewBox="0 0 20 20" fill="currentColor">
    ...
    <div class="p-6 pt-0 mt-auto">
        <div class="border-t border-steel pt-4">
            <p class="text-3xl font-extrabold text-paper">
                @if ($package->is_trial)
                    {{ __('Gratis') }}
                @else
            ...
            <form method="POST" action="{{ route('cart.store', $package) }}">
                @csrf
                <button type="submit"
                        class="flex items-center justify-center gap-2 w-full bg-brand-500 text-ink py-2.5 rounded-md hover:brightness-110 transition font-medium">
